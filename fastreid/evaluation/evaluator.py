...
class DatasetEvaluator:
    ...
    def load_module(self, model):
        """
        Preparation for generation module like "Graph Matching", "DSR distance"
        model: could be a dict or list or nn.Module
        """
        if isinstance(model, tuple) or isinstance(model, list):
            model = [module.cpu() for module in model]
