...
def build_dist(feat_1: torch.Tensor, feat_2: torch.Tensor, metric: str = "euclidean", **kwargs) -> np.ndarray:
    ...
    """
    assert metric in ["cosine", "euclidean", "jaccard", "pose"], "Expected metrics are cosine, euclidean and jaccard, " \
    ...
    if metric == "euclidean":
        ...
        return compute_pose_distance(feat_1, feat_2, qconf=kwargs["qconf"], gconf=kwargs["gconf"])
...
@torch.no_grad()
def compute_pose_distance(qfeats, gfeats, qconf, gconf):
    # qfeats.shape = [n, 2048, p]
    # gfeats.shape = [m, 2048, p]
    # qconf.shape = [n, p]
    n, m = qfeats.shape[0], gfeats.shape[0]
    dist = torch.zeros(n, m, dtype=torch.float32)
    for i in range(qfeats.shape[-1]):

        qfeat = qfeats[:, :, i]
        gfeat = gfeats[:, :, i]

        dist_m = (
                torch.pow(qfeat, 2).sum(dim=1, keepdim=True).expand(n, m)
                + torch.pow(gfeat, 2).sum(dim=1, keepdim=True).expand(m, n).t()
        )
        dist_m.addmm_(1, -2, qfeat, gfeat.t())
        # gconf[:, i]
        dist += dist_m * (1. / qfeats.shape[-1]) # * torch.sqrt(torch.einsum('n,m->nm', qconf[:, i], qconf[:, i]))
